...
(async () => {
  ...
  try {
    ...
    if (!selectedConversation.id || (childTaskName && selectedConversation.taskName !== childTaskName) || (childTaskName && !['protected', 'spawn-message'].includes(selectedConversation.assignmentSource)) || conversationMetrics.workMessages < 1 || !conversationMetrics.text.includes('도움 AI 실제 작업 기록') || /보호된 메시지|내용 없이 통신 상태|도움 AI 실행이 시작/.test(conversationMetrics.text) || conversationMetrics.focusId !== focusBeforeConversation || conversationMetrics.drawerMode !== 'subagent' || conversationMetrics.events !== selectedConversation.expectedEvents || !conversationMetrics.coordinationCollapsed || conversationMetrics.visibleTabs !== 1 || conversationMetrics.resumeAvailable !== selectedConversation.expectsResume) throw new Error(`실제 앱 도움 AI 작업 상세가 맞지 않습니다: ${JSON.stringify({ selectedConversation, conversationMetrics })}`);
