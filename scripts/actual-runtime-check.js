...
(async () => {
  ...
  try {
    ...
    const selectedConversation = await evaluate(send, `(() => { const cards = [...document.querySelectorAll('[data-open-subagent-chat]')]; const wanted = ${JSON.stringify(childTaskName)}; const card = wanted ? cards.find(node => state.snapshot.sessions.find(item => item.id === node.dataset.openSubagentChat)?.taskName === wanted) : cards[cards.length - 1]; const id = card?.dataset.openSubagentChat || ''; const session = state.snapshot.sessions.find(item => item.id === id); const expectedEvents = session ? subagentCommunicationEvents(session).length : 0; const expectsResume = Boolean(session && !isLiveSession(session) && agentResumeSupport(session).supported); card?.click(); return { id, taskName: session?.taskName || '', expectedEvents, expectsResume, clientKind: session?.clientKind || '', assignment: session?.delegation?.assignment || '', assignmentSource: session?.delegation?.assignmentSource || '' }; })()`);
    await pause(500);
    const conversationMetrics = await evaluate(send, `({ focusId: state.graphFocusId, drawerMode: state.drawerMode, events: document.querySelectorAll('[data-subagent-communication]').length, visibleTabs: document.querySelectorAll('.drawer-tab:not(.hidden)').length, resumeAvailable: Boolean(document.querySelector('[data-resume-agent]')), text: document.querySelector('.subagent-dialog-list')?.innerText || '' })`);
    const normalizedConversation = conversationMetrics.text.replace(/`/g, '');
    const normalizedAssignment = selectedConversation.assignment.replace(/`/g, '');
    if (!selectedConversation.id || selectedConversation.expectedEvents < 1 || (childTaskName && selectedConversation.taskName !== childTaskName) || (childTaskName && selectedConversation.assignmentSource !== 'parent-narration') || (childTaskName && !normalizedConversation.includes(normalizedAssignment)) || /로컬 로그에서 보호|지시 내용은 보호/.test(conversationMetrics.text) || conversationMetrics.focusId !== focusBeforeConversation || conversationMetrics.drawerMode !== 'subagent' || conversationMetrics.events !== selectedConversation.expectedEvents || conversationMetrics.visibleTabs !== 1 || conversationMetrics.resumeAvailable !== selectedConversation.expectsResume) throw new Error(`실제 앱 서브에이전트 대화 상세가 맞지 않습니다: ${JSON.stringify({ selectedConversation, conversationMetrics })}`);
